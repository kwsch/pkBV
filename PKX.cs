...
namespace pkBV
{
    public partial class PKX
    {
        ...
        public ushort mspecies;
        private ushort
            mhelditem, mTID, mSID, mTSV, mESV,
        ...
        public PKX(byte[] pkx, string ident)
        {
            ...
            try
            {
                ...
                mSpeciesName = (mnicknamestr != mSpeciesName) ? mnicknamestr + " (" + mSpeciesName + ")" : mSpeciesName;
        ...
        // Custom Functions
        internal static Image getSlotFiller(byte[] dslotdata)
        {
        ...
        internal static string getSummary(PKX pkx)
        {
            string result = "";
            result += (String.Format("{0} [{4}] lv{3} @ {1} -- {2}", pkx.Species, pkx.HeldItem, pkx.Nature, pkx.Level.ToString(), pkx.Ability) + Environment.NewLine);
            result += (String.Format("{0} / {1} / {2} / {3}", pkx.Move1, pkx.Move2, pkx.Move3, pkx.Move4) + Environment.NewLine);
            result += (String.Format(
                "IVs: {0}/{1}/{2}/{3}/{4}/{5} || " +
                "EVs: {6}/{7}/{8}/{9}/{10}/{11}",
                pkx.HP_IV.ToString("00"), pkx.ATK_IV.ToString("00"), pkx.DEF_IV.ToString("00"),
                pkx.SPA_IV.ToString("00"), pkx.SPD_IV.ToString("00"), pkx.SPE_IV.ToString("00"),

                pkx.HP_EV.ToString("00"), pkx.ATK_EV.ToString("00"), pkx.DEF_EV.ToString("00"),
                pkx.SPA_EV.ToString("00"), pkx.SPD_EV.ToString("00"), pkx.SPE_EV.ToString("00")) + Environment.NewLine);
            return result;
        }
    }
}