...
namespace pkBV
{
    public partial class Form1 : Form
    {
        ...
        private void InitLanguage()
        {
            ...
            abilitylist[0] =
                itemlist[0] =
                    movelist[0] = metXY_00000[0] = metBW2_00000[0] = metHGSS_00000[0] = "(" + itemlist[0] + ")";
        ...
        private void openBV(byte[] data)
        {
            ...
            var mode = bvtype & 0xFF;
            TB_Mode.Text = BVmode[mode];
            style = bvtype >> 8;
            TB_Style.Text = BVstyle[style];

            trainers = new string[4];
            for (int i = 0; i < 4; i++)
            {
                trainers[i] = Util.TrimFromZero(Encoding.Unicode.GetString(data, 0xEC + 0x1A*i, 0x1A));
                if (trainers[i] == "") trainers[i] = "NPC";
            }

            if (style == 4) // 4 player
                L_VS.Text = String.Format("{0} && {2} -VS- {1} && {3}", trainers[0], trainers[1], trainers[2],
                    trainers[3]);
            else
            ...
            for (int i = 0; i < 12; i++)
                pba[i].Image =
                    (Image)
                        Properties.Resources.ResourceManager.GetObject("_" +
                                                                       (previews[i] =
                                                                           BitConverter.ToUInt16(data, 0x154 + 2*i)));

            // ushort maisonNum = BitConverter.ToUInt16(data, 0x190);

            // RNG
            uint rng1 = BitConverter.ToUInt32(data, 0x1A0);
            rA.Text = rng1.ToString("X8");
            uint rng1a = BitConverter.ToUInt32(data, 0x1A4);
            r0.Text = rng1a.ToString("X8");

            ulong rng64a = BitConverter.ToUInt64(data, 0x1A8);
            rM.Text = rng64a.ToString("X16");
            ulong rng64b = BitConverter.ToUInt64(data, 0x1B0);
            rN.Text = rng64b.ToString("X16");
            ...
            // Extra Bonus!
            string u1 = Util.TrimFromZero(Encoding.Unicode.GetString(data, 0x6, 24));
            u1t.Text = u1;
            string u2 = Util.TrimFromZero(Encoding.Unicode.GetString(data, 0x50, 24));
            u2t.Text = u2;
            ...
            dat = new PKX[24];
            for (int i = 0; i < 24; i++)
                dat[i] = new PKX(pkxes[i] = PKX.decryptArray(data.Skip(0xE18 + 260*i + (i/6)*8).Take(260).ToArray()),
                    i.ToString());
            ...
            for (int i = 0; i < 24; i++)
            {
                ...
                if (i%6 == 0)
                    RTB_Team.AppendText(String.Format("{1}{2}'s Party -- {0}{1}", ++team + Environment.NewLine,
                        "========" + Environment.NewLine, trainers[team - 1]));
        ...
        private void parseBattleInstructions(byte[] data)
        {
            ...
            snip = String.Format("Instruction Length: 0x{0} ({1} bytes).", data.Length.ToString("X"), data.Length) +
                   Environment.NewLine;
            ...
            string parse = "";
            {
                ...
                if (style == 0) // Singles
                    ...
                    for (int i = 0; i < 2; i++)
                        parse +=
                            String.Format("{0} leads {1} and {2}", trainers[i], teams[i][0].Species, teams[i][1].Species) +
                            Environment.NewLine;
                else if (style < 4) // Triples
                    for (int i = 0; i < 2; i++)
                        parse +=
                            String.Format("{0} leads {1}, {2}, and {3}", trainers[i], teams[i][0].Species,
                                teams[i][1].Species, teams[i][2].Species) + Environment.NewLine;
                else
                    for (int i = 0; i < 4; i++)
                        parse += String.Format("{0} leads {1}", trainers[i], teams[i][0].Species) + Environment.NewLine;
        ...
        private void performSwitch(int Player, byte[] data, ref string parse)
        {
            ...
            for (int i = 0; i < data.Length/4; i++)
            {
                ...
                if (OpCode == 3) // Switch
                {
                    ...
                    {
                        parse += String.Format("Out: {0}, In: {1}", teams[Player][SlotOut].Species,
                            teams[Player][SlotIn].Species);
        ...
        private void performAction(int Player, byte[] data, ref string parse)
        {
            ...
            for (int i = 0; i < data.Length/4; i++)
            {
                ...
                else if (OpCode == 1) // Fight
                {
                    ...
                    parse += String.Format(Action[OpCode] + ": {3} uses {0} @ {1}{2}", movelist[move], target,
                        data[3 + 4*i] > 0 ? " - " + data[3 + 4*i] : "", teams[Player][user++].Species);
                }
                ...
                else if (OpCode == 6)
                {
                    ...
                    performRotation(ref teams[Player], Targeting);
                    parse += String.Format(Action[OpCode] + ": {1} rotates {0} to {2}", rot, curIn,
                        teams[Player][user++].Species);
                }
                else if (OpCode == 8)
                {
                    var arg = BitConverter.ToUInt16(data, 1 + 4*i).ToString("X4") + data[3 + 4*i].ToString("X2");
                    parse += String.Format(Action[OpCode] + ": {1}{2}", OpCode, Target[Targeting],
                        arg == "000000" ? "" : " - " + arg);
                }
                else
                {
